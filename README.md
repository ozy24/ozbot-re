...
[`STATS.md`](STATS.md); each snapshot runs the repro rig from a pinned, normalized nav baseline at a
fixed seed so rows isolate code changes. Add one with `py tools/benchmark.py --note "<what changed>"`.
